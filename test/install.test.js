...
check('chạy thử trên bản đã cài (file khác nội dung) không tạo .bak', () => {
  const f = path.join(dir, 'commands', 'rvpr.md')
  const before = fs.readFileSync(f, 'utf8')
  fs.writeFileSync(f, before + '\n<!-- sửa tay -->\n')
  run('--dry-run')
  const baks = fs.readdirSync(path.join(dir, 'commands')).filter((n) => n.includes('.bak-'))
  assert.deepStrictEqual(baks, [], `chạy thử mà tạo: ${baks.join(', ')}`)
  assert.ok(fs.readFileSync(f, 'utf8').endsWith('<!-- sửa tay -->\n'), 'chạy thử mà ghi đè')
  fs.writeFileSync(f, before)
})

check('rv.sh self* --base thấy cả commit trên nhánh; defbase đoán nhánh mặc định', () => {
  ...
  g(repo, 'add', '.'); g(repo, 'commit', '-q', '-m', 'a'); g(repo, 'push', '-q', 'origin', 'main')
  g(repo, 'remote', 'set-head', 'origin', 'main')
  g(repo, 'checkout', '-q', '-b', 'feat')
  fs.writeFileSync(path.join(repo, 'committed.ts'), 'x\n')
  g(repo, 'add', '.'); g(repo, 'commit', '-q', '-m', 'feat')
  fs.writeFileSync(path.join(repo, 'a.txt'), 'b\n')
  const rv = (...a) => execFileSync('bash', [path.join(dir, 'review', 'rv.sh'), '-C', repo, ...a], { encoding: 'utf8' })
  const head = rv('selfstat')
  assert.ok(head.includes('a.txt') && !head.includes('committed.ts'), head)
  const base = rv('selfstat', '--base', 'main')
  assert.ok(base.includes('a.txt') && base.includes('committed.ts'), base)
  assert.ok(rv('selfrubric', '--base', 'main').includes('module đã nạp: base typescript'), 'selfrubric --base không thấy file .ts')
  assert.strictEqual(rv('defbase').trim(), 'main')
})
